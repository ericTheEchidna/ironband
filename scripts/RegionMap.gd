...
const HEXFIELD_SCENE        := "res://scenes/HexField.tscn"
...
func _setup_hud() -> void:
	...
	_enter_btn.pressed.connect(_enter_hexfield)
...
func _set_debug_marker_hex_highlight(hex: Vector2i) -> void:
	if _mat == null:
		return
	_mat.set_shader_parameter("selection_mode", 0)
	_mat.set_shader_parameter("selected_q", hex.x)
	_mat.set_shader_parameter("selected_r", hex.y)
	_update_sel_panel("MarkerHex", "q=%d  r=%d" % [hex.x, hex.y])
...
func _enter_hexfield() -> void:
	Engine.set_meta("hex_q",        _selected_hex.x)
	Engine.set_meta("hex_r",        _selected_hex.y)
	Engine.set_meta("hex_biome_id", _get_biome_id(_selected_hex))
	get_tree().change_scene_to_file(HEXFIELD_SCENE)
